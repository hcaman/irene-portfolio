...
const TextSecondary = ({item}) => {
    ...
    return (<div className="col-sm-6 py-2"><h6>{label}: <span className="text-secondary">{text}</span></h6></div>);
};
...
const About = () => {
    ...
    const sectionOpst = { 
        ...
        mainTitle: allDatoCmsSectionsTitle.nodes[0]?.mainTitle || mainTitle || 'About',
        littleTitle: allDatoCmsSectionsTitle.nodes[0]?.littleTitle || littleTitle || 'About Me'
    };
    return (
            ...
            <div className="container">
                ...
                <div className="row align-items-center">
                    <div className="col-lg-5 pb-4 pb-lg-0">
                        <GatsbyImage className="img-fluid rounded w-100" image={image.gatsbyImageData} alt={image.alt || image.filename  || 'image'} />
                    </div>
                    <div className="col-lg-7">
                        <h3 className="mb-4">{subtitle}</h3>
                        <p>{description}</p>
                        <div className="row mb-3">
                            {itemsTextSec.map((item, i) => <TextSecondary key={i} item={item} />)}
                        </div>
